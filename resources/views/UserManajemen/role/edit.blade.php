    ...
    <div class="card mb-4">
        <div class="card-header">
            <div class="d-flex align-items-center">
                ...
                <h4 class="card-title mb-0 p-2">Edit Role</h4>
            </div>
        </div>
        <form class="card-body" action="{{ route('akun.role.update', $id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-md-12">
                    ...
                    <select id="guardName" name="guard_name" class="select2 form-select form-select-lg"
                        ...
                        <option></option>
                        @foreach ($guard as $gd)
                            <option value="{{ $gd->value }}" @if ($gd->value == $role->guard_name) selected @endif>
                                {{ strtolower($gd->name) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
